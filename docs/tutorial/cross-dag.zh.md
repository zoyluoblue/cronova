...
成功信号是持久化的：cronova 会把它与上游运行的最终状态一同提交。若全局运行
队列已满，该信号会保持待处理并在后续调度 tick 中重试；下游获准入队后，则在
`queued` 状态等待自身的 `max_active_runs` 名额。
...
整个教程到此结束——你已经从一个单独的 `echo` 任务，一路走到了带调度、感知依赖、重试加固、跨 DAG 且带告警的流水线，而这一切只需一套紧凑原生安装和一个内嵌的 SQLite 数据库。接下来可以：
