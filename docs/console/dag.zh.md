...
同一对话框还有一个可选的**优先级**字段（−100 到 100，默认 0）：优先级高的运行在竞争调度槽位时胜出；在 `serial_priority` 运行策略下，队列也按优先级从高到低出队。以非零优先级触发的运行会在 Runs 标签页和运行详情页的状态徽章旁显示一个小的 `P<n>` 徽章。
...
| Execution policy | 已有运行在执行时，排队中的运行如何准入：**并行**（按 Max active runs 并发执行——默认）、**串行等待**（同一时间只跑一个，后到的运行按逻辑时间排队）、**串行丢弃**（同一时间只跑一个，忙碌期间到来的运行会被取消——可见，绝不静默）、**串行优先**（同一时间只跑一个，队列按运行优先级从高到低出队）。串行策略强制同一时间最多一个活跃运行，无视 Max active runs。 |
