...
运行调度循环、Web 控制台与 REST API（默认 `http://localhost:8090`）。`-executor` 为空时任务在该进程内运行；托管安装会传入私有 Unix socket，把任务交给独立 executor。
...
| `-workspaces` | 系统临时目录 | 每次尝试的工程副本共享目录；托管服务会设置明确的状态目录。 |
...
| `-audit-retention` | `8760h`（365 天） | 删除早于该窗口的审计记录；`0` = 永久保留。 |
| `-max-queued-runs` | `10000` | 所有触发来源共享的全局排队运行上限。 |
| `-max-active-runs` | `1000` | 所有 DAG 共享的全局运行中上限。 |
| `-max-concurrent-tasks` | `64` | 所有 pool 共享的排队中/运行中任务上限。 |
...
反向代理部署时，应通过 `auth.trusted_proxies` 或 `CRONOVA_TRUSTED_PROXIES`
列出代理对端 IP/CIDR；其他来源提供的转发头会被忽略。
...
控制已安装的服务组。`start` 先启动 executor 再启动调度器；`stop` 停止两者；`restart` 只重启调度器，让运行中任务继续由原 executor 持有。所需进程保持健康后才会报告启动成功。
...
显示调度器和 executor 的状态。只读——从不提权；Linux 查询两个 systemd unit，macOS 查询两个 launchd label。
...
首次安装向导：HTTP 端口、绑定范围（全部网卡还是 `127.0.0.1`）、管理员账号、认证开关——每一项都可按 Enter 接受默认值。它会写入完整服务器配置，把管理员密码哈希直接写入 SQLite，并生成不含凭据的 `0600` 环境覆盖模板。重新运行时密码留空会保留当前凭据。
...
| `-env` | `cronova.env` | 要写入的不含凭据的 `0600` 环境覆盖模板（环境变量 `CRONOVA_ENV_FILE`）。 |
...
从 GitHub 下载预编译发布版，强制获取并校验 SHA256 校验和，然后原子替换两个二进制。只有未被本地修改的托管 service definition 才会自动刷新；自定义文件会保留，新版本写为 `*.dist` 供人工合并。更新会重启并检查调度器，但不会打断正在持有任务的 executor。缺少校验信息会直接终止更新。
