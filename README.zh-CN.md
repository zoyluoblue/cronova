...
**轻量、自托管的工作流调度器 —— 一条命令即可安装的开源 [Apache Airflow](https://airflow.apache.org/) / Azkaban 替代方案。**
...
- 🟢 **轻量原生安装、零服务依赖。** 纯 Go、无 CGO 的调度器 + 独立 executor，内置 SQLite。`curl | bash` 安装、`cronova update` 升级、`cronova uninstall` 卸载 —— 没有 Airflow 那套要维护的技术栈。
...
**cronova 是一款开源、自托管的工作流调度器**(即 job scheduler / task orchestrator / DAG 调度器),用 Go 编写。它按 cron 或间隔触发调度 **DAG**(任务的有向无环图),用主机自带的解释器把每个任务作为子进程运行,并提供 Web 控制台、REST API、CLI,以及给 AI 智能体用的 MCP 端点。可以把它理解为**「带依赖、重试、回填与可观测性的 cron 替代」**,或**由一组紧凑原生服务组成的轻量 Airflow 替代**。
...
|---|:---:|:---:|:---:|:---:|
| 安装 | **单发布包 / `curl \| bash`** | Python 栈 + DB + broker | JVM + MySQL | 系统自带 |
...
cronova 是**调度器,不是运行时**:它用**主机自带的解释器**(`sh`、`python3`、`java`、`psql` …)运行任务(Azkaban 风格)。托管安装会把静态调度器与静态独立 executor 部署到 **systemd(Linux)** 或 **launchd(macOS)**,无需容器或内置运行时。
...
| [控制台指南](docs/console/index.md) | Web 控制台每个页面——仪表盘、任务编辑器、运行与实时日志、资源池、令牌 |
...
是 —— 面向想要 DAG 调度(依赖、重试、catchup、资源池、Web UI、REST API),又不想跑 Python 栈、独立数据库和消息队列的团队。cronova 是一组紧凑原生服务 + 内置数据库。对于超大规模、重插件生态的数据平台,Airflow 仍是更丰富的选择。

**cronova 需要数据库、JVM 或 Python 吗?**
不需要。调度器和 Web 控制台使用内置 **SQLite**；托管安装额外运行一个轻量独立 executor，避免调度器重启杀掉任务。只有当**你的任务**要用到 Python/Java/psql 时,主机上才需要装它们。
...
**是否生产可用 / 崩溃安全?**
托管安装默认使用解耦的 gRPC executor,调度器可以在不杀掉在跑任务的前提下重启或升级;恢复时重新挂接在跑任务,不会重复执行。
