...
func (s *Store) AcquireLease(ctx context.Context, holder string, ttl time.Duration) error {
	...
	var cur string
	var expStr string
	err = tx.QueryRowContext(ctx, `SELECT holder, expires_at FROM scheduler_lease WHERE id = 1`).Scan(&cur, &expStr)
	switch {
	case errors.Is(err, sql.ErrNoRows):
		if _, err := tx.ExecContext(ctx, `INSERT INTO scheduler_lease (id, holder, expires_at) VALUES (1, ?, ?)`,
	...
	default:
		...
		if _, err := tx.ExecContext(ctx, `UPDATE scheduler_lease SET holder = ?, expires_at = ? WHERE id = 1`,
			holder, now.Add(ttl).Format(timeLayout)); err != nil {
			return err
		}
	}
...
func (s *Store) RenewLease(ctx context.Context, holder string, ttl time.Duration) error {
	res, err := s.db.ExecContext(ctx, `UPDATE scheduler_lease SET expires_at = ? WHERE id = 1 AND holder = ?`,
...
func (s *Store) ReleaseLease(ctx context.Context, holder string) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM scheduler_lease WHERE id = 1 AND holder = ?`, holder)
	return err
}

// Vacuum rebuilds the database file, returning space freed by pruned rows to
// the filesystem. Safe on a live DELETE-journal connection; used by
// `cronova prune` and `cronova backup` rather than on a schedule (it rewrites
// the whole file).
...
// BackupTo writes a consistent snapshot of the live database to destPath via
// VACUUM INTO — atomic with respect to concurrent transactions (unlike cp,
// which can capture a torn mid-commit state). destPath must not exist yet.
...
INSERT INTO task_outputs(run_id, task_id, output) VALUES (?,?,?)
...
func (s *Store) GetTaskOutput(ctx context.Context, runID, taskID string) (string, error) {
	var out string
	err := s.db.QueryRowContext(ctx, `SELECT output FROM task_outputs WHERE run_id=? AND task_id=?`, runID, taskID).Scan(&out)
...
func (s *Store) ListDagRunsByStateLimited(ctx context.Context, state model.RunState, limit int) ([]*model.DagRun, error) {
	...
	rows, err := s.db.QueryContext(ctx,
		`SELECT `+runCols+` FROM dag_runs WHERE state=? ORDER BY logical_date LIMIT ?`, string(state), limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) AppendDagVersion(ctx context.Context, dagID, hash, yaml string) error {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT hash FROM dag_versions WHERE dag_id=? ORDER BY id DESC LIMIT 1`, dagID).Scan(&last)
	...
	_, err = s.db.ExecContext(ctx,
		`INSERT INTO dag_versions(dag_id, hash, yaml) VALUES (?,?,?)`, dagID, hash, yaml)
	return err
}
...
func (s *Store) ListDagVersions(ctx context.Context, dagID string, limit int) ([]*model.DagVersion, error) {
	...
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, dag_id, hash, yaml, ts FROM dag_versions WHERE dag_id=? ORDER BY id DESC LIMIT ?`, dagID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) GetDagVersion(ctx context.Context, dagID, hash string) (*model.DagVersion, error) {
	...
	var ts string
	err := s.db.QueryRowContext(ctx,
		`SELECT id, dag_id, hash, yaml, ts FROM dag_versions WHERE dag_id=? AND hash=? ORDER BY id DESC LIMIT 1`, dagID, hash).
