...
func apiCatalog() []apiEndpoint {
	...
	return []apiEndpoint{
		...
		{Method: "POST", Path: "/api/events", Tag: "Runs",
			Summary: "Publish an external event", Desc: "Record an external event; every DAG subscribed via trigger_on_event gets one event-triggered run on the next scheduler tick. (source, key) is idempotent — re-publishing the same key cannot double-trigger, so delivery retries are safe. The payload map becomes the runs' params.",
			Request:     map[string]any{"key": "warehouse:orders_ready", "payload": map[string]string{"date": "2026-07-05"}},
			RequestDesc: "key is required (letters/digits/_.:-); payload is an optional small string map.",
			Response:    map[string]any{"accepted": true, "created": true}},
		{Method: "GET", Path: "/api/dags/{id}/task-durations", Tag: "Runs",
			Summary: "Per-task duration history", Desc: "Recent runs with per-task wall-clock durations, newest first — the data behind the console's duration-trend view.",
			Params: []apiParam{{Name: "id", In: "path", Required: true, Desc: "DAG id.", Example: "etl_daily"},
				{Name: "limit", In: "query", Desc: "Runs to include (default 30, max 100).", Example: "30"}},
			Response: map[string]any{"dag_id": "etl_daily", "runs": []any{}}},
		{Method: "GET", Path: "/api/dags/{id}/versions", Tag: "DAGs",
			Summary: "Definition history", Desc: "The DAG's append-only definition versions, newest first (each with its full YAML for diffing). Runs link to a version via their definition_hash.",
			Params:   []apiParam{{Name: "id", In: "path", Required: true, Desc: "DAG id.", Example: "etl_daily"}},
			Response: []any{map[string]any{"id": 3, "dag_id": "etl_daily", "hash": "9f2c…", "created_at": "2026-07-05T06:00:00Z"}}},
		{Method: "POST", Path: "/api/dags/{id}/versions/{hash}/restore", Tag: "DAGs",
			Summary: "Restore a definition version", Desc: "Re-register a historical definition through the normal validated create path (history is append-only — restoring adds a new version).",
			Params: []apiParam{{Name: "id", In: "path", Required: true, Desc: "DAG id.", Example: "etl_daily"},
				{Name: "hash", In: "path", Required: true, Desc: "Definition hash from the versions list.", Example: "9f2c…"}},
			Response: map[string]any{"dag_id": "etl_daily", "restored_hash": "9f2c…"}},
		{Method: "POST", Path: "/api/dags/{id}/hook", Tag: "DAGs",
			Summary: "Mint an inbound webhook", Desc: "Create (or rotate) the DAG's inbound-webhook secret. The returned path — POST /api/hooks/{id}/{secret} — triggers THIS one DAG without any bearer token; paste it into GitHub Actions or an alerting system instead of an admin credential. Shown once; only a hash is stored.",
			Params:   []apiParam{{Name: "id", In: "path", Required: true, Desc: "DAG id.", Example: "etl_daily"}},
			Response: map[string]any{"dag_id": "etl_daily", "path": "/api/hooks/etl_daily/cnv_pat_…"}},
		{Method: "GET", Path: "/api/dags/{id}/hook", Tag: "DAGs",
			Summary: "Inspect the inbound webhook", Desc: "Whether a hook exists (display prefix + creation time only; the secret is never retrievable).",
			Params:   []apiParam{{Name: "id", In: "path", Required: true, Desc: "DAG id.", Example: "etl_daily"}},
			Response: map[string]any{"exists": true, "prefix": "cnv_pat_AB12CD", "created_at": "2026-07-05T06:00:00Z"}},
		{Method: "DELETE", Path: "/api/dags/{id}/hook", Tag: "DAGs",
			Summary: "Revoke the inbound webhook", Desc: "The hook URL stops working immediately.",
			Params:   []apiParam{{Name: "id", In: "path", Required: true, Desc: "DAG id.", Example: "etl_daily"}},
		...
		{Method: "GET", Path: "/api/runs/{runID}/timeline", Tag: "Runs",
			Summary: "Run timeline", Desc: "Chronological projection of the run: lifecycle transitions, task attempts, and operator actions (audit) merged into one ordered view.",
			Params:   []apiParam{{Name: "runID", In: "path", Required: true, Desc: "Run id.", Example: "etl_daily__2026-07-05T06:00:00Z"}},
			Response: map[string]any{"run_id": "etl_daily__2026-07-05T06:00:00Z", "state": "success", "held": false, "events": []any{map[string]any{"at": "2026-07-05T06:00:00Z", "kind": "run", "event": "started"}}}},
		{Method: "POST", Path: "/api/runs/{runID}/hold", Tag: "Runs",
			Summary: "Hold a run", Desc: "Pause NEW task dispatch for an active run (running tasks continue; hold is not suspend). 409 when the run is already terminal.",
			Params:   []apiParam{{Name: "runID", In: "path", Required: true, Desc: "Run id.", Example: "etl_daily__2026-07-05T06:00:00Z"}},
			Response: map[string]any{"ok": true, "held": true}},
		{Method: "POST", Path: "/api/runs/{runID}/release", Tag: "Runs",
			Summary: "Release a held run", Desc: "Resume dispatch for a held run exactly where it paused.",
			Params:   []apiParam{{Name: "runID", In: "path", Required: true, Desc: "Run id.", Example: "etl_daily__2026-07-05T06:00:00Z"}},
			Response: map[string]any{"ok": true, "held": false}},
